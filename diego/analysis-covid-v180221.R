...
ggplot(data= simp, aes(x= Pacient, y= value)) +
  #geom_violin(trim = FALSE) + scale_fill_manual(values=c("#999999", "#E69F00"))+
  geom_boxplot() + 
  geom_jitter(shape=16, position=position_jitter(0.2), size = 2, aes(color = Pacient))
...
rownames(covid.top@tax_table@.Data) <- paste(#rownames(covid.top@tax_table@.Data),
  ...
  covid.top@tax_table@.Data[,"Species"], 
  sep = '-')
plot_heatmap(covid2, method = "NMDS", distance = "bray" )
...
entero <- subset_taxa(covid2, Genus == "Enterococcus")
## The combined object ##
g.taxa <- merge_phyloseq(veillo, prevo, kleb, rothia,
                         strep, pseudo, bacil, entero)
...
## A heatmap that shows the former mentioned bacterial genera can be depicted by this code: ##
plot_heatmap(g.taxa, method = "NMDS", distance = "bray"  )
## The heatmap at the genus level can be seen by the next code: ##
plot_heatmap(g.taxa.gen, method = "NMDS", distance = "bray" , taxa.label = "Genus" )
...
## In order to see the data at phylum level, the first two lines corresponds to the
##  relative abundances, and the third and fourth to the absolute ones 
...
covid.gene <- tax_glom(covid2,taxrank = rank_names(covid2)[6])
covid.gene.top <- filter_taxa(covid.gene, function(x) mean(x) > 10000, TRUE)

## Here, the data.frame whit the genus/phyla information is extracted, in this case is for genus
...
my.col$Age <- meta$Pacient.Age
my.col$Days.PCR_Saliva <- meta$Days.PCR_Saliva
...
my_color <- list(Saliva = c(`Negative`= "#a6cee3", `Positive`= "#33a02c", 
                 ...
                 Age = c(`<18` = "#4d9221", `20-30`= "#a1d76a",
                         ...
                         `>55` = "#c51b7d"),
                 Days.PCR_Saliva = c(`0`="#f5f5f5",`2`="#ffffd9",`5`="#c7e9b4",
                          `7`="#7fcdbb",`8`="#7fcdbb",`12`="#1d91c0",
                          `14`="#1d91c0",`37`="#225ea8", `39`="#225ea8",
                          `44`="#253494", `64`="#081d58"),
                 Same = c(`0`="#bababa", `1`="#e41a1c" , `2`="#377eb8", 
...
bef.aft <- tax_glom(bef.aft,taxrank = rank_names(covid2)[6])
bef.aft <- filter_taxa(bef.aft, function(x) mean(x) > 10000, TRUE)
