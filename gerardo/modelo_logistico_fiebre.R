...
fever_study=study_data[,7]

#Fijamos una semilla
set.seed(100)

#Elegimos conjuntos de entrenamiento y prueba
train_set<-sample(1:length(fever_study), 0.8*length(fever_study))
oligos_study_pr_train<-oligos_study_pr[train_set,]
oligos_study_pr_test<-oligos_study_pr[-train_set,]
fever_study_train<-fever_study[train_set]
fever_study_test<-fever_study[-train_set]

#Datos de estudio
data_train<-data.frame(oligos_study_pr_train)
data_test<-data.frame(oligos_study_pr_test)
fever_study_train<-factor(fever_study_train)
fever_study_test<-factor(fever_study_test)
...
fmla<-as.formula(paste("fever_study_train~",paste(xnam,collapse = "+")))

#Formulación del modelo logístico (dos clases)
null_model<-glm(formula=fever_study_train~1,family ="binomial",data=data_train)
summary(null_model)
...
confusionMatrix(data=factor(as.numeric(proba_test>0.5)),reference = fever_study_test)

#Curva ROC
curva_roc<-roc(response=fever_study_test,predictor=proba_test)
plot(curva_roc)
...
cv_net<-cv.glmnet(x=oligos_study_pr_train, y=fever_study_train,type.measure="class",alpha = alfa,family="binomial",nfolds=5)
...
model_net<-glmnet(x=oligos_study_pr_train,y=fever_study_train,family="binomial",alpha=alfa,lambda=lam)
summary(model_net)

#Predicción
fever_predict<-predict.glmnet(model_net,newx=oligos_study_pr_test,type="response")

#Error de clasificacion
x=assess.glmnet(model_net,newx=oligos_study_pr_test,newy=fever_study_test,family="response")$class

#Matriz de confusion
confusion.glmnet(model_net,newx=oligos_study_pr_test,newy=fever_study_test,family="binomial")

#Curva ROC
curva_roc<-roc(response=fever_study_test,predictor=fever_predict)
plot(curva_roc)
...
for (i in c(1:num)){
  ...
  proba_test<-predict.glm(model_forward,newdata=data_test,type = "response")
  accuracy_stepwise[i]=unname(confusionMatrix(data=factor(as.numeric(proba_test>0.5)),reference = fever_study_test)$overall[1])
  ...
  train_set<-sample(1:length(fever_study_train), 0.9*length(fever_study_train))
  oligos_study_pr_train_net<-oligos_study_pr_train[train_set,]
  oligos_study_pr_test_net<-oligos_study_pr_train[-train_set,]
  fever_study_train_net<-fever_study_train[train_set]
  fever_study_test_net<-fever_study_train[-train_set]
  
  for (j in c(1:20)){
    alfa=j/20
    #Elegimos el valor de lambda por validacion cruzada
    cv_net<-cv.glmnet(x=oligos_study_pr_train_net, y=fever_study_train_net,type.measure="class",alpha = alfa,family="binomial",nfolds=5)

    #Elegimos el valor de lambda 
    lam<-cv_net$lambda.min
    
    #Modelo logístico con la elección de alfa y lam anterior
    model_net<-glmnet(x=oligos_study_pr_train_net,y=fever_study_train_net,family="binomial",alpha=alfa,lambda=lam)
    
    #Predicción
    fever_predict<-predict.glmnet(model_net,newx=oligos_study_pr_test_net,type="response")
    
    #Error de clasificacion
    aux=unname(assess.glmnet(model_net,newx=oligos_study_pr_test_net,newy=fever_study_test_net,family="response")$class[1])
  ...
  #Error de clasificacion
  accuracy_net[i]=1-unname(assess.glmnet(model_net,newx=oligos_study_pr_test,newy=fever_study_test,family="response")$class[1])
}
...
for (i in c(1:num)){
  ...
  for (j in c(1:20)){
    ...
    #Error de clasificacion
    aux=unname(assess.glmnet(model_net,newx=oligos_study_pr_test,newy=fever_study_test,family="response")$class[1])
